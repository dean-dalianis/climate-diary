...
def initialize_routes(api: Api):
    ...
    gsom_measurement = api.model('Gsom_Measurement', {
        ...
        'days_missing': fields.Integer(required=True, description='Number of days missing'),
        'elevation': fields.Float(required=True, description='Elevation'),
        'latitude': fields.Float(required=True, description='Latitude'),
        'longitude': fields.Float(required=True, description='Longitude'),
        'more_than_once': fields.Boolean(required=False,
                                         description='Whether the measurement has been recorded more than once (applicable only to specific measurements)'),
    ...
    class ClimateData(Resource):
        ...
        def get(self):
            ...
            logger.info(f'Fetched {len(data)} climate data records.')

            if data:
                return data, 200
            else:
                return {"message": "No climate data found for the specified conditions."}, 404
