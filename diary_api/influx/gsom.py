from config import HOST, PORT, USERNAME, PASSWORD, DB_NAME, DEFAULT_MEASUREMENTS
...
def fetch_data(country_id=None, measurement=None, date=None, start_date=None, end_date=None):
    ...
    if measurement:
        if measurement not in DEFAULT_MEASUREMENTS:
            logger.error(f"Invalid measurement: {measurement}")
            return None
        measurement_names = [measurement]
    else:
        measurement_names = DEFAULT_MEASUREMENTS
